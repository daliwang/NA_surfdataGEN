...
def separate_class(input_geotiff):
    ...
    with rasterio.open(input_geotiff) as src:
        ...
        block_size = 40000  # Adjust this size based on your RAM size
            ...
            with rasterio.open(class_filename, 'w', **output_meta) as dst:
                for row_start in range(0, height, block_size):
                    ...
                    for col_start in range(0, width, block_size):
                        ...
                        window = Window(col_start, row_start, col_end - col_start, row_end - row_start)
                        data_chunk = output_sparse_data[row_start:row_end, col_start:col_end].toarray()
                        dst.write(data_chunk, window=window, indexes=1)
