...
For each Daymet gridcell, we counted the number of underlying 30~m pixels belonging to a given class. Gridcell polygons were derived from the Daymet mask geotransform. Counting was restricted to cells where the North American active-domain mask (\texttt{na\_mask.tif}) equals 1; cells with \texttt{na\_mask=0} were excluded from aggregation and assigned a fill value of \(-1\) in the per-class count rasters. For a class with ID \(k\) and a Daymet cell indexed by \(i\), the count \(c_{i,k}\) is
\[
...
\subsection{Active-Domain Mask and Inland Open Water}
The active-domain mask defines which 1~km Daymet cells participate in class counting and downstream surfdata assembly. In the initial public release (\texttt{c251230.nc}), many inland open-water cells were marked \texttt{na\_mask=0} even when the 30~m NALCMS source at the cell centre was class 18 (Water), so class-18 counts never entered the pipeline and lake fractions were incomplete over inland lakes.

For the current data record (\texttt{c260524.nc}), inland open-water cells are included upstream before aggregation. For each 1~km cell with \texttt{na\_mask=0}, the 30~m NALCMS class at the projected cell centre is sampled; if that class is 18, the cell is set to \texttt{na\_mask=1}. Class-18 counts are then re-aggregated for the opened cells, and count placeholders on newly opened cells are initialized before PFT and land-unit processing. Implementation: \texttt{NADaymet/dataProduct\_DOI/extend\_na\_mask\_open\_water.py} and \texttt{NADaymet/dataProduct\_DOI/prepare\_open\_mask\_cells.py}.
