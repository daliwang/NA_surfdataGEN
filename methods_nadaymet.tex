...
NALCMS class 19 (Snow/Ice) is mapped to a glacier land unit rather than a vegetation PFT; glacier counts are excluded from the PFT stack and handled separately with urban and lake land units (see below). NALCMS class 3 (Tropical or sub-tropical broadleaf evergreen forest) requires an additional single-threshold split at \(18\,^{\circ}\mathrm{C}\) MTCO into warm broadleaf evergreen temperate trees (PFT4, \(T > 18\,^{\circ}\mathrm{C}\)) and cool broadleaf evergreen boreal trees (PFT5, \(T \le 18\,^{\circ}\mathrm{C}\)), with count conservation enforced as for other split classes. Implementation: \texttt{ELM\_PFTs/split\_tropical\_evergreen\_18C.py}.
...
\[
  ...
  \end{cases}
\]
These are stored in \texttt{ELM\_PFT\_output/pft\_total\_count\_percentage.nc} as integer totals and float percentages (NaN fill). When summing PFT layers, fill values of \(-1\) are treated as zero so water-dominated cells do not accumulate invalid negative totals before lake, urban, and glacier counts are merged. Implementation: \texttt{ELM\_PFTs/pft\_total\_count\_percentage.py}.
...
\subsection{Final Surfdata Assembly}
Intermediate count and percentage fields are cropped and aligned to the surfdata template grid (\texttt{crop\_align\_merge.py}, offsets \(dx=98\), \(dy=72\)), merged with land-unit fractions, and written to ELM-ready variables (\texttt{PCT\_NAT\_PFT}, \texttt{PCT\_NATVEG}, \texttt{PCT\_URBAN}, \texttt{PCT\_LAKE}, \texttt{PCT\_GLACIER}, \texttt{land\_fraction}). Mean temperature of the coldest month (MTCO) is added as \texttt{AvgTempColdMonth} from \texttt{mean\_temperature\_coldest\_month.nc} (\texttt{add\_temp\_to\_surfdata.py}). Count variables and per-PFT breakdown fields are retained for traceability. The stamped release file is built with \texttt{build\_vegmap\_surfdata\_from\_combined.py}. Implementation scripts are in \texttt{NADaymet/dataProduct\_DOI/}.
