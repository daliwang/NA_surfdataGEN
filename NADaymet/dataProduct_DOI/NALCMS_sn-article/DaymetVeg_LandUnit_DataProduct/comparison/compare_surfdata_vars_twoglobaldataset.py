...
def main():
    ...
    parser.add_argument("--global-current-file", type=Path, default=Path("/gpfs/wolf2/cades/cli185/proj-shared/wangd/kiloCraft/NA_surfdataGEN/DaymetVeg_LandUnit_DataProduct/comparison/landuse.PCT_subset_last.nc"),
                        help="Path to the global 0.5° reference current landuse file.")
    parser.add_argument("--global-past-file", type=Path, default=Path("/gpfs/wolf2/cades/cli185/proj-shared/wangd/kiloCraft/NA_surfdataGEN/DaymetVeg_LandUnit_DataProduct/comparison/surfdata_0.5x0.5_simyr1850_c200609_with_TOP.subset.nc"),
                        help="Path to the 1850 surfdata file.")
    parser.add_argument("--nalcms-file", type=Path, default=Path("/gpfs/wolf2/cades/cli185/proj-shared/wangd/kiloCraft/NA_surfdataGEN/DaymetVeg_LandUnit_DataProduct/comparison/surfdata.Daymet_NA.nalcms.1km.2d.VegMapLandUnitTemp.c260122.nc"),
                        help="Path to the NALCMS-derived 1 km surfdata file.")
    parser.add_argument("--outdir", type=Path, default=Path("/gpfs/wolf2/cades/cli185/proj-shared/wangd/kiloCraft/NA_surfdataGEN/DaymetVeg_LandUnit_DataProduct/comparison"),
                        help="Directory to write output figures and stats.")
    parser.add_argument("--urban-per-layer", action="store_true", default=False,
                        help="For PCT_URBAN, compare each of 3 layers separately.")
    parser.add_argument("--urban-layer-map", type=str, default="",
                        help="Optional mapping for PCT_URBAN per-layer comparisons, format 'L:i->N:j' as '0:2,1:1,2:0'.")
    parser.add_argument("--urban-total", action="store_true", default=False,
                        help="Also plot total PCT_URBAN (sum across layers). By default, totals are plotted when --urban-per-layer is not provided.")
    args = parser.parse_args()

    out_dir = args.outdir
    out_dir.mkdir(parents=True, exist_ok=True)
    # Updated inputs: current (2016), past (1850), nalcms
    dataset_global = args.global_current_file
    dataset_daymet_1km = args.global_past_file
    ...
    dsA = load_dataset(dataset_global)       # Global (current, e.g., 2016)
    dsB = load_dataset(dataset_daymet_1km)   # Global (past, e.g., 1850)
    dsC = load_dataset(dataset_nalcms)       # NALCMS-derived (e.g., 2020)
    ...
    def compute_stats_and_plots(var_name: str, daA: xr.DataArray, daB: xr.DataArray, daC: xr.DataArray,
        ...
        # Regrid both globals (A current, B past) to the common Daymet/NALCMS grid (C)
        ...
        # Save stats
        stat_file = out_dir / f"{var_name.lower()}{suffix}_stats.txt"
        with open(stat_file, "w") as f:
            ...
            f.write("B (Global_1850 on 1 km) vs A (Global_2016 on 1 km):\n")
            ...
            f.write("C (NALCMS_2020) vs A (Global_2016 on 1 km):\n")
            ...
            f.write("C (NALCMS_2020) vs B (Global_1850 on 1 km):\n")
        ...
        # Choose plotting grid: always Daymet/NALCMS (C) grid for consistency
        x_used = xC_plot_grid if xC_plot_grid is not None else xC_grid
        y_used = yC_plot_grid if yC_plot_grid is not None else yC_grid
        kind_used = kindC_override if kindC_override is not None else kindC
        # Downsample for plotting on C grid
        x_ds, y_ds, A_plot = downsample_for_plot(x_used, y_used, A_on_C)
        _, _, B_plot = downsample_for_plot(x_used, y_used, B_on_C)
        _, _, C_plot = downsample_for_plot(x_used, y_used, daC.values)
        # Domain mask from C lon/lat
        _, _, lon_ds = downsample_for_plot(x_used, y_used, lonC)
        _, _, lat_ds = downsample_for_plot(x_used, y_used, latC)
        mask_domain = np.isfinite(lon_ds) & np.isfinite(lat_ds)
        # Color limits
        if vmin_override is not None and vmax_override is not None:
            ...
            vmax = args.map_max
        # Orientation
        A_oriented, extent_C, origin_C = prepare_imshow(A_plot, x_ds, y_ds)
        B_oriented, _, _ = prepare_imshow(B_plot, x_ds, y_ds)
        C_oriented, _, _ = prepare_imshow(C_plot, x_ds, y_ds)
        # Orient domain mask
        mask_oriented, _, _ = prepare_imshow(mask_domain.astype(float), x_ds, y_ds)
        mask_oriented = mask_oriented > 0.5
        # Maps
        maps_file = out_dir / f"{var_name.lower()}{suffix}_maps.png"
        fig, axes = plt.subplots(1, 3, figsize=(18, 6), constrained_layout=True)
        cmap_map = make_cmap("viridis")
        # Apply domain masks so only valid grid is plotted (hide zeros outside domain)
        A_masked = np.where(mask_oriented, A_oriented, np.nan)
        B_masked = np.where(mask_oriented, B_oriented, np.nan)
        C_masked = np.where(mask_oriented, C_oriented, np.nan)
        im = axes[0].imshow(np.ma.masked_invalid(A_masked), origin=origin_C, extent=extent_C, cmap=cmap_map, vmin=vmin, vmax=vmax, interpolation="nearest")
        axes[0].set_title("Global_2016")
        axes[0].set_xlabel("x" if kind_used == "xy" else "Longitude (deg)")
        axes[0].set_ylabel("y" if kind_used == "xy" else "Latitude (deg)")
        axes[1].imshow(np.ma.masked_invalid(B_masked), origin=origin_C, extent=extent_C, cmap=cmap_map, vmin=vmin, vmax=vmax, interpolation="nearest")
        axes[1].set_title("Global_1850")
        axes[1].set_xlabel("x" if kind_used == "xy" else "Longitude (deg)")
        axes[1].set_ylabel("y" if kind_used == "xy" else "Latitude (deg)")
        ...
        axes[2].set_xlabel("x" if kind_used == "xy" else "Longitude (deg)")
        axes[2].set_ylabel("y" if kind_used == "xy" else "Latitude (deg)")
        ...
        _, _, diff_BA_plot = downsample_for_plot(x_used, y_used, diff_BA)
        _, _, diff_CA_plot = downsample_for_plot(x_used, y_used, diff_CA)
        _, _, diff_CB_plot = downsample_for_plot(x_used, y_used, diff_CB)
        ...
        diff_BA_oriented, _, _ = prepare_imshow(diff_BA_plot, x_ds, y_ds)
        diff_CA_oriented, _, _ = prepare_imshow(diff_CA_plot, x_ds, y_ds)
        diff_CB_oriented, _, _ = prepare_imshow(diff_CB_plot, x_ds, y_ds)
        # Apply domain mask on C grid
        diff_BA_masked = np.where(mask_oriented, diff_BA_oriented, np.nan)
        diff_CA_masked = np.where(mask_oriented, diff_CA_oriented, np.nan)
        diff_CB_masked = np.where(mask_oriented, diff_CB_oriented, np.nan)
        diffs_file = out_dir / f"{var_name.lower()}{suffix}_diffs.png"
        fig2, axes2 = plt.subplots(1, 3, figsize=(18, 6), constrained_layout=True)
        cmap_diff = make_cmap("coolwarm")
        im2 = axes2[0].imshow(np.ma.masked_invalid(diff_BA_masked), origin=origin_C, extent=extent_C, cmap=cmap_diff, vmin=-dlim, vmax=dlim, interpolation="nearest")
        axes2[0].set_title("Global_1850 - Global_2016")
        axes2[0].set_xlabel("x" if kind_used == "xy" else "Longitude (deg)")
        axes2[0].set_ylabel("y" if kind_used == "xy" else "Latitude (deg)")
        axes2[1].imshow(np.ma.masked_invalid(diff_CA_masked), origin=origin_C, extent=extent_C, cmap=cmap_diff, vmin=-dlim, vmax=dlim, interpolation="nearest")
        axes2[1].set_title("NALCMS_2020 - Global_2016")
        axes2[1].set_xlabel("x" if kind_used == "xy" else "Longitude (deg)")
        axes2[1].set_ylabel("y" if kind_used == "xy" else "Latitude (deg)")
        axes2[2].imshow(np.ma.masked_invalid(diff_CB_masked), origin=origin_C, extent=extent_C, cmap=cmap_diff, vmin=-dlim, vmax=dlim, interpolation="nearest")
        axes2[2].set_title("NALCMS_2020 - Global_1850")
        axes2[2].set_xlabel("x" if kind_used == "xy" else "Longitude (deg)")
        axes2[2].set_ylabel("y" if kind_used == "xy" else "Latitude (deg)")
    ...
    # PCT_URBAN totals and/or per-layer
    if "PCT_URBAN" in args.vars and (args.urban_total or not args.urban_per_layer):
        out_maps = out_dir / "pct_urban_maps.png"
        out_diffs = out_dir / "pct_urban_diffs.png"
        out_stats = out_dir / "pct_urban_stats.txt"
        # Extract and total
    ...
    if "PCT_URBAN" in args.vars and args.urban_per_layer:
    ...
    for var in args.vars:
        v = var
        if v == "PCT_URBAN" and args.urban_per_layer:
            # Per-layer comparison: A=Global_2016, B=Global_1850, C=NALCMS
            if "PCT_URBAN" not in dsA.variables or "PCT_URBAN" not in dsB.variables or "PCT_URBAN" not in dsC.variables:
                print("Skipping PCT_URBAN per-layer: variable not present in one of the datasets.")
                continue
            # Identify category dims
            def find_cat_dim(da: xr.DataArray) -> str | None:
                for d in da.dims:
                    if d in {"numurbl", "density_class", "urban_class", "urbl"}:
                        return d
                return None
            catA = find_cat_dim(dsA["PCT_URBAN"])
            catB = find_cat_dim(dsB["PCT_URBAN"])
            catC = find_cat_dim(dsC["PCT_URBAN"])
            nA = dsA["PCT_URBAN"].sizes.get(catA, 0) if catA else 0
            # Parse mapping
            layer_map: dict[int, int] = {}
            if args.urban_layer_map:
                try:
                    for p in [p.strip() for p in args.urban_layer_map.split(",") if p.strip()]:
                        l, r = p.split(":")
                        layer_map[int(l)] = int(r)
                except Exception:
                    print("Warning: could not parse --urban-layer-map; using identity mapping.")
                    layer_map = {}
            # Work on common C (NALCMS) grid
            x_used = xC_grid
            y_used = yC_grid
            kind_used = kindC
            # Domain mask on C grid
            _, _, lon_ds = downsample_for_plot(x_used, y_used, lonC)
            _, _, lat_ds = downsample_for_plot(x_used, y_used, latC)
            mask_domain = np.isfinite(lon_ds) & np.isfinite(lat_ds)
            n_layers = min(3, nA if nA else 3)
            for i in range(n_layers):
                jB = layer_map.get(i, i)
                jC = layer_map.get(i, i)
                # Clamp to available ranges if dims exist
                if catB and catB in dsB["PCT_URBAN"].dims:
                    jB = max(0, min(jB, dsB["PCT_URBAN"].sizes.get(catB, jB)))
                if catC and catC in dsC["PCT_URBAN"].dims:
                    jC = max(0, min(jC, dsC["PCT_URBAN"].sizes.get(catC, jC)))
                # Extract layers
                A2 = (dsA["PCT_URBAN"].isel({catA: i}) if catA else dsA["PCT_URBAN"]).astype(np.float64).values
                B2 = (dsB["PCT_URBAN"].isel({catB: jB}) if catB else dsB["PCT_URBAN"]).astype(np.float64).values
                C2 = (dsC["PCT_URBAN"].isel({catC: jC}) if catC else dsC["PCT_URBAN"]).astype(np.float64).values
                # Regrid A2,B2 -> C grid
                A_on_C = nearest_regrid_to_target(latA, lonA, A2, latC, lonC)
                B_on_C = nearest_regrid_to_target(latB, lonB, B2, latC, lonC)
                # Downsample for plotting
                x_ds, y_ds, A_plot = downsample_for_plot(x_used, y_used, A_on_C)
                _, _, B_plot = downsample_for_plot(x_used, y_used, B_on_C)
                _, _, C_plot = downsample_for_plot(x_used, y_used, C2)
                # Scaling
                if args.scale == "robust":
                    vals = np.concatenate([A_plot.ravel(), B_plot.ravel(), C_plot.ravel()])
                    vmin = float(np.nanpercentile(vals, 2))
                    vmax = float(np.nanpercentile(vals, 98))
                else:
                    vmin, vmax = args.map_min, args.map_max
                # Orient for imshow
                A_or, extent_C, origin_C = prepare_imshow(A_plot, x_ds, y_ds)
                B_or, _, _ = prepare_imshow(B_plot, x_ds, y_ds)
                C_or, _, _ = prepare_imshow(C_plot, x_ds, y_ds)
                mask_or, _, _ = prepare_imshow(mask_domain.astype(float), x_ds, y_ds)
                mask_or = mask_or > 0.5
                # Maps
                maps_file = out_dir / f"{v.lower()}_layer{i}_maps.png"
                fig, axes = plt.subplots(1, 3, figsize=(18, 6), constrained_layout=True)
                for ax in axes:
                    ax.set_facecolor("white")
                cm = make_cmap("viridis")
                A_masked = np.where(mask_or, A_or, np.nan)
                B_masked = np.where(mask_or, B_or, np.nan)
                C_masked = np.where(mask_or, C_or, np.nan)
                im = axes[0].imshow(np.ma.masked_invalid(A_masked), origin=origin_C, extent=extent_C, cmap=cm, vmin=vmin, vmax=vmax, interpolation="nearest")
                axes[0].set_title(f"Global_2016 layer {i}")
                axes[0].set_xlabel("x" if kind_used == "xy" else "Longitude (deg)")
                axes[0].set_ylabel("y" if kind_used == "xy" else "Latitude (deg)")
                axes[1].imshow(np.ma.masked_invalid(B_masked), origin=origin_C, extent=extent_C, cmap=cm, vmin=vmin, vmax=vmax, interpolation="nearest")
                axes[1].set_title(f"Global_1850 layer {jB}")
                axes[1].set_xlabel("x" if kind_used == "xy" else "Longitude (deg)")
                axes[1].set_ylabel("y" if kind_used == "xy" else "Latitude (deg)")
                axes[2].imshow(np.ma.masked_invalid(C_masked), origin=origin_C, extent=extent_C, cmap=cm, vmin=vmin, vmax=vmax, interpolation="nearest")
                axes[2].set_title(f"NALCMS_2020 layer {jC}")
                axes[2].set_xlabel("x" if kind_used == "xy" else "Longitude (deg)")
                axes[2].set_ylabel("y" if kind_used == "xy" else "Latitude (deg)")
                cbar = fig.colorbar(im, ax=axes.ravel().tolist(), shrink=0.9)
                ...
                dCA = C2 - A_on_C
                dCB = C2 - B_on_C
                _, _, dBA_plot = downsample_for_plot(x_used, y_used, dBA)
                _, _, dCA_plot = downsample_for_plot(x_used, y_used, dCA)
                _, _, dCB_plot = downsample_for_plot(x_used, y_used, dCB)
                if args.scale == "robust":
                    dlim = float(np.nanpercentile(np.abs(np.concatenate([dBA_plot.ravel(), dCA_plot.ravel(), dCB_plot.ravel()])), 98))
                else:
                    dlim = float(args.diff_lim)
                dBA_or, _, _ = prepare_imshow(dBA_plot, x_ds, y_ds)
                dCA_or, _, _ = prepare_imshow(dCA_plot, x_ds, y_ds)
                dCB_or, _, _ = prepare_imshow(dCB_plot, x_ds, y_ds)
                dBA_masked = np.where(mask_or, dBA_or, np.nan)
                dCA_masked = np.where(mask_or, dCA_or, np.nan)
                dCB_masked = np.where(mask_or, dCB_or, np.nan)
                diffs_file = out_dir / f"{v.lower()}_layer{i}_diffs.png"
                fig2, axes2 = plt.subplots(1, 3, figsize=(18, 6), constrained_layout=True)
                cd = make_cmap("coolwarm")
                im2 = axes2[0].imshow(np.ma.masked_invalid(dBA_masked), origin=origin_C, extent=extent_C, cmap=cd, vmin=-dlim, vmax=dlim, interpolation="nearest")
                axes2[0].set_title(f"Global_1850 layer {jB} - Global_2016 layer {i}")
                axes2[1].imshow(np.ma.masked_invalid(dCA_masked), origin=origin_C, extent=extent_C, cmap=cd, vmin=-dlim, vmax=dlim, interpolation="nearest")
                axes2[1].set_title(f"NALCMS_2020 layer {jC} - Global_2016 layer {i}")
                axes2[2].imshow(np.ma.masked_invalid(dCB_masked), origin=origin_C, extent=extent_C, cmap=cd, vmin=-dlim, vmax=dlim, interpolation="nearest")
                axes2[2].set_title(f"NALCMS_2020 layer {jC} - Global_1850 layer {jB}")
                for ax in axes2:
                    ax.set_xlabel("x" if kind_used == "xy" else "Longitude (deg)")
                    ax.set_ylabel("y" if kind_used == "xy" else "Latitude (deg)")
                cbar2 = fig2.colorbar(im2, ax=axes2.ravel().tolist(), shrink=0.9)
            ...
            continue
        if v == "PCT_URBAN":
            # skip here; totals/per-layer handled separately if needed
            continue
