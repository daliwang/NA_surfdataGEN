...
def extent_from_xy(x: np.ndarray, y: np.ndarray):
    return [float(np.min(x)), float(np.max(x)), float(np.min(y)), float(np.max(y))]


def prepare_paper_imshow(z, x_1d: np.ndarray, y_1d: np.ndarray):
    """Match PaperFigures maps: south-to-north row flip with origin='lower' on Daymet x/y."""
    z_plot = z[::-1, :]
    extent = extent_from_xy(x_1d, y_1d)
    return z_plot, extent, "lower"


def downsample_array(z: np.ndarray, max_points: int = 1_000_000) -> np.ndarray:
    ny, nx = z.shape
    stride = compute_stride_for_plot(ny, nx, max_points=max_points)
    return z[::stride, ::stride]


def style_paper_map_axes(ax) -> None:
...
def save_comparison_figures(
    ...
    """Write comparison maps. layout='split' -> 2-panel maps + separate diff; layout='triple' -> NALCMS | LULC | diff."""
    if paper_style:
        xlabel = "X coordinate"
        ylabel = "Y coordinate"
    else:
        xlabel = "x" if kindN == "xy" else "Longitude (deg)"
        ylabel = "y" if kindN == "xy" else "Latitude (deg)"
    ...
    if layout == "triple":
        panel_file = outdir / f"{stem}_lulc2020_vs_nalcms_panel.png"
        fig_h = 7.5 if paper_style else 6.2
        ...
        im_map = axes[0].imshow(
            np.ma.masked_invalid(N_masked), origin=origin, extent=extent,
            cmap=cmap, vmin=vmin, vmax=vmax, interpolation="nearest",
            aspect="equal" if paper_style else None,
        )
        ...
        axes[1].imshow(
            np.ma.masked_invalid(L_masked), origin=origin, extent=extent,
            cmap=cmap, vmin=vmin, vmax=vmax, interpolation="nearest",
            aspect="equal" if paper_style else None,
        )
        axes[1].set_title(f"LULC_2020 {var_label}", pad=10)
        if paper_style:
        ...
        im_diff = axes[2].imshow(
            np.ma.masked_invalid(diff_masked), origin=origin, extent=extent,
            cmap=diff_cmap, vmin=-dlim, vmax=dlim, interpolation="nearest",
            aspect="equal" if paper_style else None,
        )
        axes[2].set_title(f"NALCMS - LULC_2020", pad=10)
        if paper_style:
    ...
    maps_file = outdir / f"{stem}_lulc2020_vs_nalcms_maps.png"
    ...
    axes[0].set_title(f"LULC_2020 {var_label}", pad=10)
    axes[0].set_xlabel(xlabel)
    axes[0].set_ylabel(ylabel)
    axes[1].imshow(
        np.ma.masked_invalid(N_masked), origin=origin, extent=extent,
    ...
    axes[1].set_title(f"NALCMS {var_label}", pad=10)
    axes[1].set_xlabel(xlabel)
    axes[1].set_ylabel(ylabel)
    ...
    diffs_file = outdir / f"{stem}_lulc2020_vs_nalcms_diffs.png"
    ...
    ax2.set_title(f"NALCMS - LULC_2020", pad=10)
...
def main():
    ...
    parser.add_argument("--layout", choices=["split", "triple"], default="split",
                        help="'split' writes 2-panel maps + separate diff file; 'triple' writes one row (NALCMS | LULC 2020 | difference).")
    parser.add_argument(
        "--paper-style",
        action="store_true",
        help="Use Daymet projected X/Y coordinates and orientation matching other paper figures.",
    )
    ...
    def finalize_plot_layers(L_plot, N_plot, diff_plot, maskN, xN_ds, yN_ds):
        if args.paper_style:
            L_or, extent, origin = prepare_paper_imshow(L_plot, x_1d, y_1d)
            N_or, _, _ = prepare_paper_imshow(N_plot, x_1d, y_1d)
        ...
        mask_or = mask_or > 0.5
        return L_or, N_or, diff_or, mask_or, extent, origin
    ...
    for v in args.vars:
        ...
        if v == "PCT_URBAN":
            ...
            if args.paper_style:
                L_plot = downsample_array(L_on_N, max_points=args.max_points)
                N_plot = downsample_array(vN_eff, max_points=args.max_points)
                stride = compute_stride_for_plot(vN_eff.shape[0], vN_eff.shape[1], max_points=args.max_points)
                xN_ds = xN_grid[::stride, ::stride]
                yN_ds = yN_grid[::stride, ::stride]
            else:
                xN_ds, yN_ds, L_plot = downsample_for_plot(xN_grid, yN_grid, L_on_N)
        ...
        if args.paper_style:
            stride = compute_stride_for_plot(lonN.shape[0], lonN.shape[1], max_points=args.max_points)
            lonN_ds = lonN[::stride, ::stride]
            latN_ds = latN[::stride, ::stride]
            xN_ds = xN_grid[::stride, ::stride]
            yN_ds = yN_grid[::stride, ::stride]
        ...
        L_or, N_or, diff_or, mask_or, extent, origin = finalize_plot_layers(
            L_plot, N_plot, diff_plot, maskN, xN_ds, yN_ds
        )
