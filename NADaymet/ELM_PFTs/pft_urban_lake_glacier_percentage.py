...
glacier_file = 'ELM_PFT_output/glacier_landtype19_nalcms_Snow_Ice_in_daymet.nc'

# Output file
output_file = 'ELM_PFT_output/combined_pft_urban_lake_glacier_total_count.nc'
...
with Dataset(pft_file, 'r') as nc:
    ...
    # Load glacier count and percentage if present
    glacier_count = nc.variables['glacier_count'][:] if 'glacier_count' in nc.variables else None
    glacier_percentage = nc.variables['glacier_percentage'][:] if 'glacier_percentage' in nc.variables else None
...
# Compute total count
Total_count = pft_total_count + urban_arr + lake_arr + glacier_count
...
with Dataset(output_file, 'w', format='NETCDF4') as nc:
    ...
    nc.createVariable('glacier_count', 'i2', ('file', 'y', 'x'), zlib=True, complevel=5)[:] = glacier_count
    ...
    nc.createVariable('glacier_percentage', 'f4', ('file', 'y', 'x'), zlib=True, complevel=5, fill_value=np.nan)[:] = glacier_percentage
