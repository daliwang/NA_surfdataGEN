...
with Dataset(glacier_file, 'r') as nc:
    glacier_arr = nc.variables['glacier_count'][:]

# Compute total count
Total_count = pft_total_count + urban_arr + lake_arr + glacier_arr
...
with np.errstate(divide='ignore', invalid='ignore'):
    ...
    glacier_percentage = np.where(Total_count > 0, (glacier_arr / Total_count) * 100.0, np.nan)
...
with Dataset(output_file, 'w', format='NETCDF4') as nc:
    ...
    nc.createVariable('glacier_count', 'i2', ('file', 'y', 'x'), zlib=True, complevel=5)[:] = glacier_arr
